...
var file = await fs.open(filePath, "w");
var dataArr = [];
for (let m = 1; m < 12; m++) {
  const url2 = `https://www.timeanddate.com/sun/bulgaria/sofia?month=${m}&year=${year}`;
  const res = await fetch(url2);
  if (!res.ok) {
    throw new Error(`Error fetching url - ${url2}`);
  }
  const html3 = await res.text();
  const $2 = load(html3);
  const data2 = $2("#as-monthsun > tbody > tr").map((_, e) => {
    if ($2(e).find("td").length < 5)
      return;
    return {
      month: m,
      day: +$2(e).attr("data-day"),
      start: $2(e).find("td:nth-child(10)").text().replace(/[^\d+.\d+].*/, ""),
      end: $2(e).find("td:nth-child(11)").text().replace(/[^\d+.\d+].*/, "")
    };
  }).get();
  dataArr.push(...data2);
}
